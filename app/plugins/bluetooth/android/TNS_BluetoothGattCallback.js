...
var TNS_BluetoothGattCallback = (function (_super) {
    ...
    TNS_BluetoothGattCallback.prototype.onInit = function (owner) {
        this._owner = owner;
        common_1.CLog(common_1.CLogTypes.info, "---- TNS_BluetoothGattCallback.onInit ---- this.owner: " + this._owner);
    };
    TNS_BluetoothGattCallback.prototype.onConnectionStateChange = function (gatt, status, newState) {
        common_1.CLog(common_1.CLogTypes.info, "---- TNS_BluetoothGattCallback.onConnectionStateChange ---- status: " + status + ", newState: " + newState);
        if (newState === android.bluetooth.BluetoothProfile.STATE_CONNECTED &&
            status === android.bluetooth.BluetoothGatt.GATT_SUCCESS) {
            common_1.CLog(common_1.CLogTypes.info, '---- discovering services -----');
            gatt.discoverServices();
        }
        else {
            common_1.CLog(common_1.CLogTypes.info, "---- disconnecting the gatt: " + gatt + " ----");
            this._owner.get().gattDisconnect(gatt);
        }
    };
    TNS_BluetoothGattCallback.prototype.onServicesDiscovered = function (gatt, status) {
        common_1.CLog(common_1.CLogTypes.info, "---- TNS_BluetoothGattCallback.onServicesDiscovered ---- status (0=success): " + status);
            ...
            for (var i = 0; i < services.size(); i++) {
                ...
                for (var j = 0; j < characteristics.size(); j++) {
                    ...
                    for (var k = 0; k < descriptors.size(); k++) {
                        ...
                        common_1.CLog(common_1.CLogTypes.info, "pushing descriptor: " + descriptor);
                        descriptorsJs.push(descriptorJs);
                    }
                    var characteristicJs = {
                        UUID: this._owner.get().uuidToString(characteristic.getUuid()),
                        name: this._owner.get().uuidToString(characteristic.getUuid()),
                        properties: {
                    ...
                    common_1.CLog(common_1.CLogTypes.info, "pushing characteristic: " + characteristicJs);
    ...
    TNS_BluetoothGattCallback.prototype.onCharacteristicRead = function (gatt, characteristic, status) {
        var device = gatt.getDevice();
        common_1.CLog(common_1.CLogTypes.info, "----- TNS_BluetoothGattCallback.onCharacteristicRead ---- characteristic: " + characteristic + ", status: " + status + ", device: " + device);
        var stateObject = this._owner.get().connections[device.getAddress()];
        if (!stateObject) {
            this._owner.get().gattDisconnect(gatt);
            return;
